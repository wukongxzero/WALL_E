    ...
    if (Serial.available()) {
        ...
        switch (cmd) {
            case 'd':
                ...
                //Serial.println(debugStream ? "Debug stream ON" : "Debug stream OFF");
                if (debugStream)
                    //Serial.println("ms,driveL,driveR,lPWM,rPWM,pitch,pkts");
                break;
            case 'b':
                debugPackets = !debugPackets;
                //Serial.print("Packet debug: ");
                //Serial.println(debugPackets ? "ON" : "OFF");
                break;
            case 'p':
                printState();
                break;
            case 's':
                stopMotors();
                rxStatus.driveLeft  = 0;
                rxStatus.driveRight = 0;
